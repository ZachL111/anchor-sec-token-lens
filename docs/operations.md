...
`anchor-sec-token-lens` is centered on security tooling work in PHP. The technical target is: Implement a PHP security tooling project for token policy evaluation, using deny and allow fixtures and explainable decision traces.
...
The score is deterministic. The current threshold is 182. Risk has a penalty of 4, latency has a penalty of 4, and weight contributes a bonus of 3.
